...
const checksum = (array) => {
  const result = [];
  array.forEach(item => item.sort((a, b) => b - a));
  array.forEach(sorted => result.push(sorted[0] - sorted[sorted.length - 1]));
  return result.reduce((sum, current) => sum + current);
};

const evenlyDivisible = (array) => {
  const result = [];
  array.forEach(item => item.sort((a, b) => b - a));
  array.forEach((item) => {
    for (let i = 0; i < item.length; i += 1) {
      for (let j = i + 1; j < item.length; j += 1) {
        while (item[i] / 2 < item[j]) j += 1;
        if (item[i] % item[j] === 0) {
